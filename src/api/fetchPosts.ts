  ...
  try {
    ...
    const posts: Post[] = await Promise.all(
      tweets.map(async (tweet) => {
        ...
        // Mapper les réponses
        const replies: Reply[] = (repliesData.data || []).map((reply) => {
          const user = repliesData.includes.users.find(
            (u) => u.id === reply.author_id,
          );
          return {
            id: reply.id,
            displayName: user ? user.name : "Utilisateur inconnu",
    ...
    if (error instanceof Error) {
      ...
      throw new Error("Erreur inconnue lors de la récupération des posts");
    }
  }
};
